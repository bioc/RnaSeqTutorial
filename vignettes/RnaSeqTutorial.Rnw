...
\Rpackage{GenomicAlignments} packages. Then, annotation will be retrieved
...
\Rpackage{Rsamtools} and \Rpackage{GenomicAlignments} packages capabilities.
...
The SAM/BAM format \citep{Li:2009p1662} is a \emph{de-facto}
standard for storing NGS aligned data. As a consequence, the
\Rpackage{Rsamtools} was developed to import the \emph{samtools}
functionalities into the R environment. SAM/BAM files can be sorted by chromosomal
...
create an index for the BAM file and then read the data using
\Rpackage{Rsamtools}.
...
A flexible way to load BAM formatted files is to
...
\paragraph{GenomicAlignments}
Finally, the last example to load data uses the
\Rpackage{GenomicAlignments} package. With the increasing read length, it
...
purpose. The \Rpackage{GenomicAlignments} package was implemented to deal with
...
\Rpackage{GenomicAlignments} package functionalities.
...
As for \Rpackage{biomaRt}, coercing the obtained object is
...
\item \textbf{Q1}: There is indeed no Q1, no need to look for it.
It has been removed for Bioc 3.2.

\item \textbf{Q2}: section \ref{question2}, page \pageref{question2}: The \Robject{aln3}, of class
\Rclass{GAlignments} from the \Rpackage{GenomicAlignments} package
...
or simply:
<<solution 5b, eval=FALSE>>=
exon.grange2 <- as(gInterval,"GRanges")
@

which means that a \Rclass{RangedData} object can be obtained as:
<<solution 5c, eval=FALSE>>=
